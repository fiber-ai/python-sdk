...
T = TypeVar("T", bound="PersonCommentedOnPost")


@_attrs_define
class PersonCommentedOnPost:
    """
    Attributes:
        type_ (Literal['person_commented_on_post']):
        ...
        keywords (list[str] | None | Unset): Only alert for comments where the comment text, post content (when
            available from provider), or post author name matches these keywords. Omit for any comment.
    """
    ...
    def from_dict(cls: type[T], src_dict: Mapping[str, Any]) -> T:
        d = dict(src_dict)
        type_ = cast(Literal["person_commented_on_post"], d.pop("type"))
        if type_ != "person_commented_on_post":
            raise ValueError(f"type must match const 'person_commented_on_post', got '{type_}'")
        ...
        person_commented_on_post.additional_properties = d
        return person_commented_on_post
